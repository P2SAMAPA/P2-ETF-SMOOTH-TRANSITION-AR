...
def load_data(token: str | None = None) -> pd.DataFrame:
    ...
    # Same pattern as working SJM/GP repos
    if isinstance(df.index, pd.DatetimeIndex):
        df = df.reset_index().rename(columns={"index": "Date"})
    df["Date"] = pd.to_datetime(df["Date"])
    df = df.sort_values("Date").reset_index(drop=True)

    # Data is closing prices — ffill then compute log returns
    prices = df.set_index("Date").ffill()
...
def upload_results(local_path: Path | str, remote_path: str, token: str) -> None:
    """Upload a file to the HF results dataset using HfApi.upload_file."""
